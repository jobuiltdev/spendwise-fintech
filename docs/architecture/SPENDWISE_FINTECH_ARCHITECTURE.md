...
## 17F. Decisions closed in M5 — transfers

M5 answers a fourth question, kept apart from the first three: *which bank
transfer is the customer making?* The ledger still owns what money exists (M2),
the hold what is reserved (M3), and the financial transaction the lifecycle
(M4). A transfer owns business intent and nothing else.

M5 adds exactly two things M4 could not do alone: a **destination**, prepared
atomically with the transaction and reservation that back it; and **transfer
accounting** — the balanced entries that make a successful transfer true, which
M4 deliberately would not invent because it does not know what a transfer is.

| # | Decision | Detail |
|---|---|---|
| M5-1 | **Four concepts, four modules, no blending** | Journal = what posted. Hold = what is reserved. FinancialTransaction = what is happening. Transfer = which bank transfer is intended. `Transfer` and `FinancialTransaction` are **not** collapsed into one model. |
| M5-2 | **`Transfer → OneToOne → FinancialTransaction`, and the transfer owns the relationship** | Exactly one transaction per transfer and one transfer per transaction. The column lives on `Transfer`, so `FinancialTransaction` gains **no** transfer-specific field and stays usable for future non-transfer operations. A test asserts the only thing M5 added to it is the reverse accessor. |
| M5-3 | **Wallet, amount and currency are NOT stored on `Transfer`** | They already exist on the transaction as immutable intent, and a second copy is a second thing that can disagree. They are read-through properties instead. This is stronger than validating a mismatch: **a transfer whose principal differs from its transaction's amount is not a state this model can represent**, so no equality has to be enforced anywhere. |
| M5-4 | **The destination is an immutable snapshot, not a reference** | `recipient_name`, `destination_account_number`, `destination_bank_code`, `destination_bank_name`, stored on the transfer. A saved recipient could later be edited and a bank's display name could change; a transfer reading through to a mutable record would then describe an attempt that never happened. History stays truthful because it is a copy. |
| M5-5 | **No saved beneficiary model** | The snapshot is sufficient, so no `Beneficiary`, `Recipient`, `Bank` or `BankAccount` model exists. Saved recipients are a product feature that can arrive later **without touching transfer history**, precisely because history does not reference them. |
| M5-6 | **Verification is a boundary, not an implementation** | `VerifiedBankAccount` is a frozen, self-validating value object. Constructing one is the *caller's assertion* that the account name came from a trustworthy resolution. **M5 provides no way to obtain that resolution**: no lookup, no bank directory, no name-enquiry function and no endpoint claiming to verify an account. Faking verification would be worse than not offering it; M6's provider abstraction supplies the real (or simulated) thing. |
| M5-7 | **Ten-digit account numbers are the CURRENT NGN product rule, not a moneycore invariant** | NUBAN account numbers are ten digits and SpendWise's first rail is Nigerian bank transfer, so both the domain and a database check constraint enforce `^[0-9]{10}$`. This is documented as a rail-specific product rule: a future rail with a different account format needs a destination type of its own, not a loosened column. |
| M5-8 | **Bank code is an opaque validated string** | Shape only — short, printable, no whitespace. M5 knows **no provider's bank-code vocabulary**, bundles no bank list, and ships no directory. Mapping our codes to a provider's is M6's (O-28). |
| M5-9 | **Surrounding whitespace is stripped; internal whitespace in an account number is refused, not repaired** | Copy-and-paste routinely carries surrounding whitespace, so rejecting it is pedantry. Being lenient about the *digits* is how money reaches the wrong person, so a value that is not already exactly right is rejected rather than fixed up. Recipient and bank names keep their internal spacing exactly as given — a person's name is not ours to reformat. Control characters and line breaks are refused everywhere. |
| M5-10 | **Outgoing only** | Receiving money and account funding need provider and webhook knowledge that does not exist yet. M4 already supports incoming *transactions* generically; fabricating an incoming transfer flow to exercise that would be inventing a product surface. Incoming transfer modelling stays open (O-30). |
| M5-11 | **`Transfer` has no status field** | `transfer.status` reads through to the transaction. There is no `transfer_status`, `bank_status` or `payment_status` column — a second lifecycle is a second thing that can disagree with the first. |
| M5-12 | **No cancellation, no draft, no abandonment** | Carried forward from the locked §8.5 and M4-12. **Preparing a transfer is the point at which it is accepted for execution**, not the point at which the customer starts typing — which is what makes "no cancellation" workable without a draft. Backing out before acceptance produces no financial transaction at all. If persisted drafts are ever needed they belong to the transfer/product layer outside the money engine, and **M5 must keep that separation** (O-31). |
| M5-13 | **Preparation is atomic: transfer, transaction and reservation, all or none** | There is no state in which a transaction exists without its hold, or a hold exists without the transfer it was reserved for. Proved by forcing a failure at each step in turn and asserting nothing survives. |
| M5-14 | **Idempotency reuses M4's mechanism and extends it with the destination** | M4's key identifies `(wallet, direction, amount, currency, key)`. **A transfer's intent also includes where the money is going.** The same key with the same amount and a *different* destination — account number, bank code, bank display name, recipient name or narration — raises `transfer_idempotency_conflict` instead of quietly returning the earlier transfer, because quietly returning it would send money to the wrong person. This is the critical M5 invariant. No competing idempotency framework was built. |
| M5-15 | **A conflict names the fields, never the other destination** | A caller replaying a key that is not theirs is told *which* fields differ, not where that money went. |
| M5-16 | **A key already naming a non-transfer transaction is refused** | If the transaction under a key has already been started, resolved, or given a reservation by a direct M4 caller, preparation raises `transfer_transaction_mismatch` rather than adopting it. An untouched `CREATED` transaction with matching wallet, direction, amount and currency **is** adopted — that is what the key is for, and refusing it would strand the key with no way to complete. |
| M5-17 | **Lock order unchanged: wallet first** | `prepare_transfer` takes the `Wallet` row lock at the top using M2's ascending-primary-key helper, then transaction, then hold, then transfer, then ledger posting on success. **No path takes a transaction, hold or transfer lock before a wallet lock**, so M5 introduces no new deadlock cycle. Holding the wallet lock for the whole of preparation is also what makes concurrent duplicate preparation resolve cleanly rather than race. |
| M5-18 | **Reservation is principal-only** | `hold.amount_minor == transfer.amount_minor`. Transfer fees do not exist and M5 invents none (O-25). M4 already permits a hold larger than the principal, so a future fee policy fits without changing anything here. No default expiry: O-17 stays open, so no timeout window is guessed and no scheduler exists. |
| M5-19 | **M5 owns transfer accounting; M4 posts what it is given** | `transfer_principal_entries` derives the balanced entries and `succeed_transfer` hands them to M4's `succeed_transaction`. The release, the posting and the status change all happen inside **M4's** single atomic block — M5 adds no second orchestration, never releases a hold by hand and never creates a journal by hand. Asserted by source inspection as well as by behaviour. |
| M5-20 | **The principal invariant, stated as an effect rather than a direction** | A successful transfer must reduce the **wallet's posted normal balance by exactly the transfer principal** — no more, no less — checked against M2's `_wallet_balance_deltas` before anything is posted. This closes the M4 limitation that success could in principle post accounting unrelated to the transaction's amount. |
| M5-21 | **Success is account-type neutral** | O-13 is open, so "transfer out = debit the wallet" is not a rule M5 is entitled to write down. The entry side is **derived** from the account's actual `account_type`: a debit-normal wallet account is reduced by a **credit**, a credit-normal one by a **debit**. Both are proved end-to-end, and no default wallet classification was introduced. |
| M5-22 | **The counterpart is supplied by the caller, and must be internal** | M5 invents **no settlement account taxonomy** — which internal account a transfer faces depends on the custody and settlement design and stays open (O-29). What M5 does decide is that the counterpart must not be a customer wallet: a bank transfer leaves the SpendWise wallet system, and an internal wallet-to-wallet movement is a different product operation that would need its own design. Currency must match, and a journal touching any wallet other than the sender's is refused. |
| M5-23 | **No `start_transfer`** | M6 is the first milestone allowed to begin provider execution, and a function here that pretended to start one would be fabricating execution. Reaching `PROCESSING` goes through M4's `start_processing` until then. M5 prepares only. |
| M5-24 | **Failure and UNKNOWN are thin wrappers, not a second state machine** | `fail_transfer` and `mark_transfer_unknown` validate that the transaction really belongs to the transfer and delegate to M4 unchanged. No provider error mapping, no failure vocabulary, no timeout handling. **UNKNOWN keeps the reservation**; there is no transfer-level pending, confirming or awaiting-confirmation state, and "Confirming" remains the product's word for the transaction's `unknown`. |
| M5-25 | **Transfer intent is immutable and transfers are never deleted** | Destination, narration and the transaction link cannot be rewritten through `save()`; queryset `update()` and `delete()` are refused outright, as is model `delete()`. A transfer to a different recipient is a **new transfer**, not an edit. Financial history persists even when the transfer failed. |
| M5-26 | **Narration is short, bounded and part of the intent** | 100 characters, no control characters, immutable with the transfer, and included in the idempotency comparison. Never a place for provider data. |
| M5-27 | **No customer API** | Provider execution, account verification and transaction authorization are all incomplete, so exposing a transfer endpoint would present a production-ready surface that cannot work. No transfer, beneficiary, bank or verify-account route is registered for any HTTP method, no serializer or view names a transfer, and the M1 `GET /api/financial-account/` response is unchanged and still leaks nothing — verified with a live prepared transfer. |
| M5-28 | **Migration `0005` is purely additive; `0001`–`0004` are byte-unchanged** | One `CreateModel` and four check constraints. No data migration, no `RunPython`, no `RunSQL`, and no ledger account seeded. Verified forward, reversed to `0004`, and re-applied on PostgreSQL. |
| M5-29 | **No transaction PIN, KYC, risk, limits or fees** | M9 owns transaction security, M10 risk. M5 validates only what it can know: positive integer amount, matching currency, a well-formed destination, sufficient available balance, and its own domain invariants. **No Nigerian regulatory limit is guessed** — a test asserts no five-digit numeric literal exists anywhere in the transfer modules. |

### What M5 deliberately did not build

Provider abstraction, a provider simulator, a bank API client, account
verification, a recipient-lookup endpoint, a bank directory, provider
references or statuses, webhooks, reconciliation, a settlement engine, a fee
engine, transfer limits, KYC, risk scoring, a transaction PIN, incoming
transfers, a customer transfer endpoint, mobile transfer UI, Celery, Redis, an
outbox, scheduled or recurring transfers, transfer drafts, cancellation, a
...
## 17H. Decisions closed in M7 — webhooks and status recovery

M6 ends with two kinds of unresolved truth: a `STARTED` attempt that may or may
not have reached the rail, and an `UNKNOWN` one whose answer never came back.
M7 answers the question those leave open — *how does SpendWise learn what
actually happened?* — from evidence, and never by sending anything again.

| # | Decision | Detail |
|---|---|---|
| M7-1 | **Recovery is observational. There is no submission path at all** | The one question M7 may ask a rail is "what became of this request I already made?". `provider_recovery` is typed against a Protocol with **no submission method**, imports nothing that submits, and is proved by AST walk to call no `submit_transfer`, `execute_transfer`, `submit` or `resubmit`. An unresolved operation may already have moved a customer's money; resending is the one action that turns an ambiguity into a double debit. |
| M7-2 | **Evidence is appended; history is never rewritten** | `ProviderExecutionAttempt` remains M6's immutable record of what *one interaction observed*. An attempt that came back `unknown` stays `unknown` forever, even once a webhook proves the transfer succeeded — because that is what was observed at the time. What resolves is the **financial transaction**, justified by an appended `ProviderRecoveryEvidence` row. "We did not know, then we were told" reads differently from "it succeeded all along", and only the first is true. |
| M7-3 | **`STARTED` stays `STARTED`, and is never finalised by later evidence** | The alternative — letting recovery close out an attempt that never captured an immediate result — would erase the distinction between an immediate response and a later discovery. That distinction is the whole reason M6's crash window is documented rather than papered over, so the attempt keeps its `finished_at` null and the evidence carries the rest of the story. Asserted directly. |
| M7-4 | **An unresolved lookup never turns `STARTED` into `UNKNOWN`** | They record different historical facts: one interaction that never finished, versus one that finished inconclusively. Collapsing them would lose information for no gain. |
| M7-5 | **Three status outcomes, and unresolved is its own type** | `ProviderTransferStatusSucceeded`, `…Failed`, `…Unresolved`. No boolean success flag anywhere, for M6's reason: a bool cannot distinguish "definitely not" from "we do not know". Impossible states stay unrepresentable — a success has no `failure_code` field, an unresolved cannot borrow a failure code as its reason. |
| M7-6 | **A rail answering "not found" is UNRESOLVED, not failure** | For a request that may never have arrived, "no record" establishes nothing: it is equally an indexing delay, eventual consistency, or a key the rail does not index. Treating it as failure would release a reservation on money that may already have gone. `NOT_FOUND_IS_DEFINITIVE = False` records this, and an adapter may only override it if a real rail's documented semantics make it definitive. None does today, so the question stays open. |
| M7-7 | **Unresolved evidence changes nothing financial** | No transition, no hold release, no journal, no new attempt. It is still recorded: that SpendWise asked and could not be told is worth keeping. |
| M7-8 | **Definitive evidence resolves through M4/M5, never directly** | `succeed_transfer` and `fail_transfer` do the work. Recovery assigns no transaction status, creates no journal and releases no hold — asserted by AST over assignment targets, since `status ==` is a comparison and `status =` is the defect. |
| M7-9 | **Verification and parsing are one inseparable operation** | `verify_and_parse_webhook(body, headers)`. Splitting them would leave a `parse` that application code could call on unverified bytes, which is an invitation. A delivery that does not authenticate raises before anything is read from the database. |
| M7-10 | **Authentication happens outside every transaction and lock** | Signature checking never holds a wallet or transaction lock, proved at runtime the way M6 proves its own boundary. |
| M7-11 | **An unauthenticated delivery is not an event** | Nothing is stored, nothing resolved, no attempt touched, no balance moved. There is deliberately no `INVALID` webhook status: persisting rejected requests would give an attacker a way to fill the table. The rejection reveals nothing about the expected signature. |
| M7-12 | **Two models, and deliberately not one** | `ProviderWebhookEvent` is a *transport receipt* — proof an authenticated delivery arrived, unique on `(provider_key, provider_event_id)`, and able to exist **unmatched**. `ProviderRecoveryEvidence` is a *financial observation about one attempt*, from either source, and always has an attempt. A matched webhook produces both, because "this arrived" and "this is what we now believe" have different lifetimes and different uniqueness rules. |
| M7-13 | **Webhook idempotency is the rail's own event identity, and is invariant to delivery timing** | `(provider_key, provider_event_id)` unique. An identical delivery — same identity, same normalised contents — resolves money at most once and every caller is told it was accepted, **whether the deliveries arrived sequentially or concurrently**. Timing is an accident of the network and is not part of what an event means, so concurrency must never turn a duplicate into a conflict. On losing the unique-constraint race the service re-reads the winning receipt inside a savepoint-isolated insert and compares contents: identical means idempotent, different means conflict. Proved with 2- and 5-way concurrent deliveries producing one receipt, one evidence row, one journal and one hold release, for success and failure alike, and mutation-verified against the pre-correction behaviour. No content-hash idempotency was invented as a universal rule; a rail that cannot supply an event id is an adapter-specific problem for whichever milestone integrates it. |
| M7-14 | **A reused event id with different contents is a conflict, not a redelivery** | This is the only thing that makes a webhook conflict, and it holds concurrently as well as sequentially. Compared on the immutable normalised intent — provider key, event id, outcome, client reference, provider reference, failure code — and never on local handling metadata such as processing status, timestamps or row id. The original receipt is never overwritten, no second receipt is created, and no second financial mutation occurs. |
| M7-15 | **Contradictory definitive evidence is escalated, never resolved** | Success then failure — or the reverse — raises `provider_recovery_conflict`. Choosing a winner would mean fabricating a reversal or discarding evidence. **No journal is reversed, no balance adjusted, no compensating entry created**, and the settled outcome is untouched. The contradicting evidence is kept and the receipt marked `CONFLICTED`. Operations and reconciliation decide; that is M8's. |
| M7-16 | **The conflict record commits before the error is raised** | Found during implementation: raising from inside the atomic block rolled back the very evidence that had to be kept. Both paths now return the conflict to their caller, which raises after the transaction commits. Discarding a contradiction is precisely what must not happen. |
| M7-17 | **Same-outcome evidence after resolution is a safe no-op** | Redeliveries and repeated queries are expected, so agreeing evidence returns quietly rather than raising. Only disagreement is an error. |
| M7-18 | **Status queries are observational and may repeat** | Asking moves no money. Repeated lookups append repeated observations and change nothing else. **No polling cadence is defined** and no scheduler exists — how often to look is a production decision nobody has made. `attempts_awaiting_recovery()` lists what needs attention and deliberately does not act. |
| M7-19 | **Correlation is by `client_reference` first, corroborated by provider reference** | SpendWise owns the client reference. If both are present and name different attempts, that is `provider_recovery_reference_mismatch` — refused, because guessing which to believe is how evidence gets attached to the wrong customer's money. **Matching never falls back to amount, account number or recipient**: a plausible-looking match is not a match. |
| M7-20 | **`client_reference` is now unique on the attempt** | The one schema touch on an M6 table, added additively in `0007`. M7 correlates evidence back to an execution by this value, so the lookup must be provably unambiguous rather than merely improbable. M6's values were already unique by construction — a digest of immutable transaction identity, one attempt per transaction — so the constraint adds no behaviour and rejects no existing row; it turns an assumption M7 depends on into a guarantee the database enforces. |
| M7-21 | **An authentic webhook naming nothing we have is kept, not invented around** | Recorded as `UNMATCHED` and raised as `provider_webhook_unmatched`. It never creates a transaction and is never attached to the nearest plausible one. An authenticated rail describing money we have no record of is exactly what operations must see. |
| M7-22 | **Recovery evidence and receipts are append-only** | Outcome, source, attempt, event id, references and failure code are all immutable; bulk update and delete are refused on evidence, delete on receipts. Only a receipt's `processing_status` moves, because that is our handling of a delivery rather than what the rail said. |
| M7-23 | **Nothing raw is persisted** | No payload, body, headers, signature, credential, HTTP status or digest — asserted field by field on both models. What is not normalised is not evidence. Whether raw retention is ever required for compliance is a production and legal decision, and stays open. |
| M7-24 | **Canonical lock order unchanged, financial rows before evidence rows** | `Wallet (ascending PK) → FinancialTransaction → ProviderExecutionAttempt → recovery evidence / webhook receipt → hold / transfer / ledger work`, taken explicitly in full rather than left to a nested M4/M5 call. Proved by spying on the issued `FOR UPDATE` statements, including that no evidence row is ever locked before a financial one. |
| M7-25 | **No HTTP route** | Routing a webhook would require provider discovery and configuration for a partner who has not been chosen, so ingestion stays at the application-service level. `ingest_webhook(provider, body, headers, …)` takes exactly what an HTTP view would hand it, so adding the view later is a thin adapter rather than a redesign. Every candidate route still 404s. |
| M7-26 | **No scheduler, no queue, no Celery, no Redis** | Recovery is synchronous and invoked by the application. Nothing about correctness requires background processing: an unresolved attempt simply stays unresolved, with its funds reserved, until someone looks. |
| M7-27 | **No reconciliation, no auto-correction, no manual ops action** | No statement import, no balance comparison, no compensating entry, and no mark-successful / force-fail / release-funds action. M8 owns reconciliation; M11 owns operations tooling. |
| M7-28 | **The simulator's status behaviour is configured independently of submission** | `status_scenario` is separate from `transfer_scenario`, which is what makes the milestone's story tellable: a submission that came back ambiguous, later found to have succeeded, with no resubmission. Deterministic throughout — no randomness, no sleeping, no I/O. |
| M7-29 | **The webhook signing scheme is the simulator's own, and says so** | A fixed, published, deliberately worthless test secret named `…-not-for-production`, injectable per instance, with constant-time comparison. **No environment variable, no settings key, no vendor scheme.** No provider has been chosen, so there is no production secret to hold and pretending otherwise would be worse than holding none. |

### What M7 deliberately did not build

Transfer resubmission or retry of any kind, a second provider attempt, a real
provider integration, production webhook secrets or configuration, an HTTP
webhook route, provider-specific signature schemes, a polling scheduler, Celery,
Redis, an outbox, reconciliation, settlement, statement import, balance
adjustment or compensating entries, journal reversal, manual operations
...
| O-17 | **Default hold duration.** M3 sets none: `expires_at` is optional and has no default, because how long a reservation should live depends on transfer timeout windows and provider reservation semantics that are not decided | §17D M3-4, §17F M5-18 |
...
| O-28 | **Provider bank-code mapping.** M5 treats a bank code as an opaque validated string and ships no directory; which vocabulary SpendWise adopts and how it maps to a provider's is M6's | §17F M5-8 |
| O-29 | **The settlement counterpart taxonomy.** M5 owns how a transfer principal maps into balanced entries but invents no production settlement account — the caller supplies the counterpart, and which internal account is correct follows from the custody and provider arrangement | §17F M5-22, §17C O-13 |
| O-30 | Whether incoming transfers are modelled as transfer objects at all, and what receives them. M5 is outgoing-only; M4 already supports incoming transactions generically | §17F M5-10 |
...
| O-35 | **UNKNOWN resolution strategy. Narrowed by M7, not closed:** truth now arrives by provider status query or authenticated webhook, both observational and neither ever resubmitting. **What stays open** is the production mechanism — cadence (O-41), routing (O-43), and which real provider supplies either | §17G M6-6, §17H M7-1 |
| O-36 | **Crash-window recovery policy. Narrowed by M7 to "query, never resubmit":** a `STARTED` attempt is resolved by asking, stays `STARTED` whatever the answer, and is never converted to a failure or resent. **What stays open** is how long to wait and what to do when the query is itself ambiguous — today it simply remains unresolved with funds reserved | §17G crash window, §17H M7-3 |
| O-37 | Provider timeout values and transport configuration. No partner exists, so any number would be a guess | §17G M6-29 |
| O-38 | Provider retry rules, if a rail's own idempotency guarantees ever make a safe resubmission possible. **M6 and M7 both forbid resubmission outright**, and M7 adds an AST-level proof that recovery contains no submission path | §17G M6-11, §17H M7-1 |
| O-39 | Real webhook signature validation. **Narrowed by M7:** the verification *boundary* exists and inbound-event idempotency is settled — `(provider_key, provider_event_id)` unique. **What stays open** is the real scheme, which is a provider's to specify | §17G M6-28, §17H M7-9, M7-13 |
...
| O-41 | Production polling cadence for unresolved attempts — how often to look, and by what trigger. M7 makes the sweep possible (`attempts_awaiting_recovery`) and deliberately schedules nothing | §17H M7-18 |
| O-42 | Per-provider NOT_FOUND semantics. M7 treats it as unresolved for every rail; whether any real provider makes it definitive is that adapter's question to answer with evidence | §17H M7-6 |
